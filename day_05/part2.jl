...
function parse_lines(input)
    ...
    if long == true
        ...
        new_seeds = Int[]
        # println("seed new loop")
        for (i, s) in enumerate(seeds[begin:2:end])
            p = 2*(i-1)+1
            # println(p)
            # println(s)
            range_seeds = [s+k for k in range(0,seeds[p+1]-1)]
            # println(range_seeds)
...
function process_maps_fast(map_matrix, seeds, maps)
    println("Smarter processing, but fast")
    locations = Int[]
    ...
    for (start_seed, num_seeds) in zip(seeds[begin:2:end], seeds[begin+1:2:end])
        println("start seed: ",start_seed, "\t num seeds: ", num_seeds)
        # println("seed ", seed)
        
        conversion = start_seed
        for key in maps
            ...
            for j in 1:r
                ...
                range_max = start_seed + num_seeds - 1
                
                println("is range max greater than dest_max")

                new_seeds = [start_seed+k for k in range(0,num_seeds-1)]
                if range_max > dest_max

                    println(range_max, " ? ", dest_max)
                else
                    for conversion in new_seeds
                        shift = conversion-dest
                        # println("\trow")
                        if conversion < dest_max && conversion >= dest
                            println(dest,"-", dest_max)
